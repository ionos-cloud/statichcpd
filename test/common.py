...
class server_process:
    ...
    def __enter__(self):
        ...
        self.process = NSPopen(
            self.nsname, bashcommand.split(), stdout=subprocess.PIPE
        )
        return self
...
class Config:
    def __init__(self, arg_dict={}):
        ...
        self.clients = [
            Interface(name=name)
            for name in ["vethclient" + str(idx) for idx in range(self.count)]
        ]
        self.servers = [
            Interface(name=name)
            for name in ["vethserver" + str(idx) for idx in range(self.count)]
        ]
        if (
            "server_ips" in arg_dict
            and len(arg_dict["server_ips"]) >= self.count
        ):
        ...
        with open(self.conf_file, "a") as f1:
            ...
            if all(
                re.match("^vethserver.*", intf.name) for intf in self.servers
            ):
                ...
                f1.write(
                    "served_interface_regex = " + arg_dict.get("regex") + "\n"
                )
    ...
    def __enter__(self):
        ...
        try:
            for i in range(self.count):
                ...
                self.ip.link(
                    "add",
                    ifname=self.clients[i].name,
                    kind="veth",
                    peer=self.servers[i].name,
                )
                ...
                idx = self.ip.link_lookup(ifname=self.servers[i].name)[0]
                self.ip.addr(
                    "add", idx, address=self.servers[i].addr4, mask=24
                )
...
def getHwAddr(ifname: str) -> str:
    ...
    info = fcntl.ioctl(
        sock.fileno(), 0x8927, struct.pack("256s", bytes(ifname[:15], "utf-8"))
    )
...
def init_client_table(mac_list: List[str], ifname_list: List[str]) -> None:
    ...
    for i in range(len(mac_list)):
        ...
        cursor.execute(
            " insert into clients (ifname, mac) values (?,?)", (ifname, mac)
        )
    conn.commit()
    cursor.close()


def init_host_conf_table(
    mac_list: List[str],
    ifname_list: List[str],
...
def init_v6_tables(
    ...
    ifname_list: List[str],
    clients: List[Interface],
    v6_attr_lists: List[List[Tuple[str, str]]],
) -> None:
    cursor = conn.cursor()
    ...
    for i in range(len(mac_list)):
        ...
        for attr in attr_list:
            cursor.execute(
                """ insert into client_v6configuration
...
def build_default_v6conf(clients: List[Interface]):
    ...
    for idx in range(len(clients)):
        ...
        default_v6conf[clients[idx].name] = [
            (3, client_ip6),
            (4, client_ip6),
            (25, client_v6prefix),
        ]
...
def build_default_v4conf(clients: List[Interface]):
    ...
    for idx in range(len(clients)):
        ...
        default_v4conf[clients[idx].name] = [
            (256, client_ip),
        ...
        for i in range(10, 13):
            default_v4conf[clients[idx].name].extend(
                [
                    (
                        dhcp.DHCP_OPT_ROUTER,
                        str((idx + 1) * 10) + ".0.0." + str(i),
                    )
                ]
            )
