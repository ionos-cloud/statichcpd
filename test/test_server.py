            ...
            with server_process(cfg) as srv:
                ...
                for (client, server) in zip(cfg.clients, cfg.servers):
                    ...
                    with dhcp_sender("-v " + client.name) as sender:
                        sleep(
                            2
                        )  # Give some time for the address to get configured
                        client_idx = cfg.ip.link_lookup(ifname=client.name)[0]
                        client_ip = cfg.ip.get_addr(index=client_idx)[
                            0
                        ...
                        try:
                            ...
                            print(
                                "Client ",
                                client.name,
                                " succesfully configured with IP address",
                                client_ip,
                            )
                            ...
                            self.fail(
                                "Client "
                                + client.name
                                + " failed to get succesful IP binding"
                            )

                    # DHCPv4 Renew state
                    with dhcp_sender(
                        "-v " + client.name + " -s " + server.addr4
                    ) as sender:
                        sleep(1)
                        client_ip = cfg.ip.get_addr(index=client_idx)[
                            0
                        ...
                        try:
                            ...
                            print(
                                "Client ",
                                client.name,
                                " succesfully renewed IP address ",
                                client_ip,
                            )
                        except AddressValueError:
                            self.fail(
                    ...
                    with dhcp_sender("-v -6 " + client.name) as sender:
                        sleep(
                            2
                        )  # The solicit-advertisement-request-reply will take some time
                        client_ip = None
                        for addr in cfg.ip.get_addr(
                            index=client_idx, family=AF_INET6
                        ):
                            if addr["scope"] == 0:
                                client_ip = cfg.ip.get_addr(
                                    index=client_idx, family=AF_INET6
                                )[0].get_attr("IFA_ADDRESS")
                                break
                        try:
                            ...
                            print(
                                "Client ",
                                client.name,
                                " succesfully configured with IPv6 address ",
                                client_ip,
                            )
                        except AddressValueError:
                            self.fail(
