...
valid_single_valued_attr: Dict[str, int] = {
    "Subnet Mask": (dhcp.DHCP_OPT_NETMASK, dtype.IPV4.value),
...
valid_multi_valued_attr: Dict[str, int] = {
    "Router": (dhcp.DHCP_OPT_ROUTER, dtype.IPV4.value),
...
def init_client_table(mac_list: List[str], ifname_list: List[str]) -> None:
    ...
    for i in range(len(mac_list)):
        ...
        cursor.execute(
            " replace into clients (ifname, mac) values (?,?)", (ifname, mac)
        )
    conn.commit()
    cursor.close()
...
def init_host_conf_table(
    ...
    for i in range(len(mac_list)):
        ...
        for attr in attr_list:
            cursor.execute(
                """ replace into client_configuration
...
attr_lists = [
    [
        (valid_single_valued_attr["IPv4"][0], "20.0.0.1"),
        (valid_single_valued_attr["Subnet Mask"][0], "255.255.255.0"),
        (
            valid_single_valued_attr["Time Offset"][0],
            0xFFFFD5D0,
        ),  # Use hex to denote negative time offset
        ...
        (
            valid_multi_valued_attr["Classless Static Route"][0],
            "30.1.0.0/16,30.1.0.1",
        ),
        (
            valid_multi_valued_attr["Classless Static Route"][0],
            "20.10.10.0/24,20.10.10.1",
        ),
    ],
    [
...
v6_attr_lists = [
    [(3, "20::1"), (3, "20::2"), (4, "40::1"), (25, "20::/64")],
    [(3, "30::1"), (3, "30::2"), (4, "50::1"), (25, "30::/64")],
]
