...
def validate_msg(msg: Message.ClientServerDHCP6, server_duid: bytes) -> bool:
    if msg.mtype in [SOLICIT, CONFIRM, REBIND]:
        ...
        return (
            ...
            and (
                ...
                or fetch_dhcp6_opt(msg, DHCP6_OPT_SERVERID) == server_duid
            )
        )
    return True
...
def construct_dhcp6_packet(
    msg: Message.ClientServerDHCP6,
    msg_type: int,
    opt_list: Tuple[Tuple[int, Any], ...],
) -> Message.ClientServerDHCP6:
...
def construct_ia_na_response_data(
    ...
    ia_na_opts = fetch_all_dhcp6_opt(
        msg, DHCP6_OPT_IA_NA
    ...
    for requested_na in ia_na_opts:
        ...
        logger.debug(
            "Client: %s Ifname: %s IA_NA requested for IAID: %s",
            client_id,
            ifname,
            requested_id,
        )
        logger.debug(
            "Client: %s Ifname: %s Available IA_NA configurations for the client: %s",
            client_id,
            ifname,
            conf_data,
        )
        ...
        if disable_ia_id:
            ...
            if not all(isinstance(ele, IPv6Address) for ele in conf_data):
                logger.error(
                    "Client %s: Input Error: Invalid data for client's IA_NA"
                    " when disable_ia_id is set: %s",
                    client_id,
                    conf_data,
                )
                return encoded_value
            configured_addr6_list = cast(
                List[IPv6Address], conf_data
            )  # To satisfy mypy
        else:
            # Consider only the set of addresses that have the same ID
            if (
                not all(isinstance(ele, tuple) for ele in conf_data)
                or not all(
                    isinstance(val, str)
                    for val in [ele[0] for ele in conf_data]  # type: ignore
                )
                or not all(
                    isinstance(val, IPv6Address)
                    for val in [ele[1] for ele in conf_data]  # type: ignore
                )
            ):
                logger.error(
                    "Client: %s : Input Error: Invalid data for client's IA_NA"
                    " when disable_ia_id is not set: %s",
                    client_id,
                    conf_data,
                )
                return encoded_value
            configured_addr6_list = [
                val[1]
                for val in cast(Iterable[Tuple[str, IPv6Address]], conf_data)
        ...
        encoded_value += t1.to_bytes(4, "big") + t2.to_bytes(
            4, "big"
        )  # Append t1,t2 to the IA_NA value
        ...
        if len(requested_na) > 12:  # Len > 12 => IA_NA options are present
            ...
            while offset < len(requested_na):
                ...
                if op == DHCP6_OPT_IAADDR:
                    try:
                        ...
                        logger.error(
                            "Client:%s Ifname:%s Invalid IPv6 address in IAADDR option: %s",
                            client_id,
                            ifname,
                            requested_na[offset + 4 : offset + 20],
                        )
                ...
                offset += (
                    ...
                    + 2
                    + struct.unpack(
                        ">H", requested_na[offset + 2 : offset + 4]
                    )[0]
                )
            logger.debug(
                "Client: %s Ifname: %s Requested list of IAADDRs is %s",
                client_id,
                ifname,
                requested_addr_list,
            )
            ...
            expired_addresses = [
                ...
                if addr not in configured_addr6_list
            ]
            logger.debug(
                "Client: %s Ifname: %s Expired address list: %s",
                client_id,
                ifname,
                expired_addresses,
            )
...
def construct_ia_ta_response_data(
    ...
    ia_ta_opts = fetch_all_dhcp6_opt(
        msg, DHCP6_OPT_IA_TA
    )  # There could be multiple IA_NA options
    encoded_value = b""
    if conf_data is None:
    ...
    for requested_ta in ia_ta_opts:
        ...
        if disable_ia_id:
            ...
            if not all(isinstance(ele, IPv6Address) for ele in conf_data):
                logger.error(
                    "Client: %s Input Error: Invalid data for client's IA_TA"
                    " when disable_ia_id is set: %s",
                    client_id,
                    conf_data,
                )
        ...
        if len(requested_ta) > 4:  # Len > 4 => Options are present
            ...
            while offset < len(requested_ta):
                ...
                offset += (
                    ...
                    + 2
                    + struct.unpack(
                        ">H", requested_ta[offset + 2 : offset + 4]
                    )[0]
                )
            logger.debug(
                "Client: %s Ifname: %s Requested list of temporary IAADDRs is %s",
                client_id,
                ifname,
                requested_addr_list,
            )
            ...
            logger.debug(
                "Client: %s Ifname: %s Expired temporary address list: %s",
                client_id,
                ifname,
                expired_addresses,
            )
...
def construct_ia_pd_response_data(
    ...
    ia_pd_opts = fetch_all_dhcp6_opt(
        msg, DHCP6_OPT_IA_PD
    )  # There could be multiple IA_PD options
    if conf_data is None:
        return None
    encoded_value = b""
    ...
    for requested_pd in ia_pd_opts:
        ...
        logger.debug(
            "Client: %s Ifname: %s IA_PD requested for ID: %s",
            client_id,
            ifname,
            requested_id,
        )
        logger.debug(
            "Client:%s Ifname:%s Available IA_PD configurations for the client: %s",
            client_id,
            ifname,
            conf_data,
        )
        ...
        if disable_ia_id:
            ...
            if not all(isinstance(ele, IPv6Network) for ele in conf_data):
                logger.error(
                    "Client: %s Input Error: Invalid data for client's prefix"
                    " delegation when disable_ia_id is set: %s",
                    client_id,
                    conf_data,
                )
                return encoded_value
            configured_pd_list = cast(
                List[IPv6Network], conf_data
            )  # To satisfy mypy
        else:
            if (
                not all(isinstance(ele, tuple) for ele in conf_data)
                or not all(
                    isinstance(val, str)
                    for val in [ele[0] for ele in conf_data]
                )
                or not all(
                    isinstance(val, IPv6Network)
                    for val in [ele[1] for ele in conf_data]
                )
            ):
                logger.error(
                    "Client: %s Input Error: Invalid data for client's prefix"
                    " delegation when disable_ia_id is not set: %s",
                    client_id,
                    conf_data,
                )
                return encoded_value
            ...
            configured_pd_list = [
                val[1]
                for val in cast(Iterable[Tuple[str, IPv6Network]], conf_data)
                if val[0] == requested_id
            ]
        ...
        if len(requested_pd) > 12:  # Len > 12 => IA_PD options are present
            ...
            while offset < len(requested_pd):
                ...
                if op == DHCP6_OPT_IAPREFIX:
                    try:
                        ...
                        requested_pd_list.extend(
                            [IPv6Network(network_addr + "/" + prefix_len)]
                        )
                    except AddressValueError:
                        logger.error(
                            "Client: %s Ifname: %s Invalid IPv6 values in IAPREFIX option: %s",
                            client_id,
                            ifname,
                            requested_pd[offset + 13 : offset + 29],
                        )
                ...
                offset += (
                    ...
                    + 2
                    + struct.unpack(
                        ">H", requested_pd[offset + 2 : offset + 4]
                    )[0]
                )
            logger.debug(
                "Client: %s Ifname:%s Requested list of IAADDRs is %s",
            ...
            expired_prefixes = [
                ...
                if prefix not in configured_pd_list
            ]
            logger.debug(
                "Client:%s Ifname:%s Expired address list: %s",
                client_id,
...
def append_mandatory_options(
    ...
    if client_duid is not None:  # To satisfy mypy
        opt_list.extend(
            [
                (DHCP6_OPT_SERVERID, server_duid),
            ...
            if isinstance(data, IPv6Address):
                ...
                if all(isinstance(ele, IPv6Address) for ele in data):
                    if opcode == DHCP6_OPT_IA_NA:
                        encoded_data = construct_ia_na_response_data(
                            msg, data, host_conf_data, ifname, client_id
                        )
                    elif opcode == DHCP6_OPT_IA_TA:
                        encoded_data = construct_ia_ta_response_data(
                            msg, data, host_conf_data, ifname, client_id
                        )
                    else:
                    ...
                    encoded_data = b"".join(
                        [elem.encode("utf-8") for elem in data]
                    )
                elif all(isinstance(ele, IPv6Network) for ele in data):
                    encoded_data = construct_ia_pd_response_data(
                        msg, data, host_conf_data, ifname, client_id
                    )
                elif all(
                    isinstance(ele, tuple) for ele in data
                ):  # Case of IA_NA, IA_PD or IA_TA values
                    if opcode == DHCP6_OPT_IA_NA:
                        encoded_data = construct_ia_na_response_data(
                            msg, data, host_conf_data, ifname, client_id
                        )
                        ...
                        logger.error(
                            "Client:%s Ifname:%s Configuration data for opcode %d "
                    ...
                    logger.error(
                        "Client:%s Ifname:%s Elements of unexpected datatype "
                ...
                logger.error(
                    "Client:%s Ifname:%s Value(%s) of unexpected type "
                    "received for opcode %d",
                    client_id,
                    ifname,
                    data,
                    opcode,
...
def construct_dhcp_reply(
    ...
    server_duid: bytes,
    client_id: Union[Mac, str],
) -> Optional[Message.ClientServerDHCP6]:
    opt_list = construct_dhcp6_opt_list(
        msg, request_list_opt, ifname, host_conf_data, client_id
    )
    if (
        not opt_list
    ):  # If no other parameters were requested by client, should offer be sent?
        logger.debug(
            "Client:%s Ifname:%s No configuration data found",
            client_id,
            ifname,
        )
        return None
...
def fetch_client_duid(client_duid: Optional[bytes]) -> str:
    ...
    if (
        duid_type == DUID.lladdr_time.value
    ):  # Static HCPD server cannot server client DUID with dynamic value (time)
        client_id = "".join(
            str(elem) for elem in struct.unpack(">HH", client_duid[:4])
        ) + ll_addr(client_duid[8:])
        logger.debug(
            "Unable to serve dynamic client DUID %s."
            " Removing time from the duid value for DB lookup: %s",
            client_duid,
            client_id,
        )
        ...
        client_id = (
            "".join(str(elem) for elem in struct.unpack(">H", client_duid[:2]))
            + "".join(
                str(elem)
...
def process_solicit_msg(
    ...
    logger.debug(
        "%s used as client ID for database lookup for packet on %s",
        client_id,
        ifname,
    )

    host_conf_data, server_iface = fetch_host_conf_data(
        DHCPv6DB(), ifname, client_id
    )
    if not host_conf_data:
        logger.debug(
            "Client: %s Ifname: %s No configuration data found for the host. Skipping ..",
            client_id,
            ifname,
        )
    ...
    if rapid_commit:
        ...
        dhcp_response = construct_dhcp_adv(
            ifname,
            msg,
            request_list_opts,
            host_conf_data,
    ...
    if not dhcp_response:
        logger.debug(
            "Client:%s Ifname:%s No response DHCP6 Advertise packet for %s ",
...
def process_request_renew_rebind_info_msg(
    ifname: str,
    msg: Message.ClientServerDHCP6,
    server_duid: bytes,
    src_mac: Mac,
    ...
    dhcp_response = construct_dhcp_reply(
        ifname, msg, request_list_opts, host_conf_data, server_duid, client_id
    )

    if not dhcp_response:
        logger.debug(
            "Client:%s Ifname:%s No DHCP6 Reply packet for %s ",
            client_id,
            ifname,
            msg.mtype,
        )
...
def process_confirm_msg(
    ...
    if DHCP6_OPT_IA_NA in opts:
        encoded_ia_na = construct_ia_na_response_data(
            msg,
            host_conf_data.get(DHCP6_OPT_IA_NA, None),
            host_conf_data,
            ifname,
            client_id,
        )
    if DHCP6_OPT_IA_TA in opts:
        encoded_ia_ta = construct_ia_ta_response_data(
            msg,
            host_conf_data.get(DHCP6_OPT_IA_TA, None),
            host_conf_data,
            ifname,
            client_id,
        )
    ...
    opt_list: Tuple[Tuple[int, bytes], ...] = (
        (DHCP6_OPT_STATUS_CODE, struct.pack(">H", DHCP6_Success)),
    )
    ...
    if encoded_ia_na is None or encoded_ia_ta is None:
        opt_list = (
            (DHCP6_OPT_STATUS_CODE, struct.pack(">H", DHCP6_NotOnLink)),
        )
    opt_list = append_mandatory_options(msg, opt_list, server_duid)
    ...
    if not dhcp_response:
        logger.debug(
            "Client:%s Ifname:%s No response DHCP6 Reply packet for %s",
            client_id,
            ifname,
            msg.mtype,
        )
...
def process_client_server_msg(
    ...
    if not valid_msg:
        logger.error(
            "%s message validation failed on interface %s from source mac %s.",
            dhcp6_type_to_str(msg.mtype),
            ifname,
            src_mac,
        )
    ...
    logger.debug(
        "Ifname:%s SrcMac: %s Received a client server message of type %s",
        ifname,
        src_mac,
        dhcp6_type_to_str(msg.mtype),
    )
    ...
    if (
        msg.mtype is REQUEST
        or msg.mtype is RENEW
        or msg.mtype is REBIND
        or msg.mtype is INFORMATIONREQUEST
    ):
        return process_request_renew_rebind_info_msg(
            ifname, msg, server_duid, src_mac
        )
...
def process_relayforw_msg(
    ifname: str,
    payload: Message.RelayServerDHCP6,
    server_duid: bytes,
    src_mac: Mac,
) -> Tuple[Optional[bytes], Optional[str]]:
    err_return_val: Tuple[None, None] = (None, None)
    ...
    if dhcp_msg is None:
        logger.debug(
            "Empty DHCP Message in DHCP Relay Forward msg from %s on intf %s",
            src_mac,
            ifname,
        )
        return err_return_val
    reply_msg, server_iface = process_client_server_msg(
        ifname, Message.ClientServerDHCP6(dhcp_msg), server_duid, src_mac
    )
    if reply_msg is None:
        logger.debug(
            "Empty DHCP Message in DHCP Relay Reply to %s on intf %s",
            src_mac,
            ifname,
        )
        return err_return_val
...
def process_relay_server_msg(
    ...
    err_return_val: Tuple[None, None] = (None, None)
    logger.debug(
        "Received a relay server message of type %s from %s",
        dhcp6_type_to_str(payload.mtype),
        src_mac,
    )
    ...
    if use_mac_as_duid:
        logger.error(
            "Server configured to use source mac as client DUID. Unable to handle %s from relay agent(%s).",
            dhcp6_type_to_str(payload.mtype),
            src_mac,
        )
        return err_return_val
