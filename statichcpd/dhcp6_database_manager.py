...
def insert_data_from(csv_filename: str) -> None:
    ...
    with open (csv_filename, 'r') as f:
        reader = csv.reader(f)
        columns = next(reader)
        query = 'insert into valid_v6attributes({0}) values ({1})'.format(','.join(columns), ','.join('?' * len(columns)))
        for row in reader:
            # Opcode can be an integer or statichcpd.dhcp6 module optcode alias
            try:
...
def init(config: SectionProxy) -> None:
    ...
    insert_data_from('/usr/share/statichcpd/default_v6attr.csv')
...
def fetch_v6host_conf_data(ifname: str, duid: Mac) -> Dict[str,Any]:
    ...
    for (opcode, max_count, datatype, value) in cursor.execute(
                     """ select valid_v6attributes.opcode, valid_v6attributes.max_count,
                         ...
                         where ifname=? and duid=?""", (ifname, str(duid))
                     ):
        try:
            ...
            logger.error("Invalid datatype entry %d for opcode %d "
                         "for client (%s, %s)",
                         datatype.value, opcode, ifname, duid)
            cursor.close()
        ...
        if max_count == 1:                           ## Assumed that application handles insertion of attr values
            if datatype is dtype.IPV6:               ## appropriately
